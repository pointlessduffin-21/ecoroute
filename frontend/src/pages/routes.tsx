...
export function RoutesPage() {
  ...
  // Find the most recently auto-generated route (sorted by createdAt descending)
  const lastAutoGeneratedRoute = useMemo(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="mx-4 md:mx-6 mb-3 rounded-lg border border-[#1da253]/30 bg-[#1da253]/5 p-3">
        <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            <Zap className="h-4 w-4 text-[#1da253]" />
            <span className="text-sm font-medium text-foreground">
              Auto-generation is active
            </span>
            <span className="text-xs text-muted-foreground">
              -- routes are created automatically when bins exceed threshold
            </span>
          </div>
          {lastAutoGeneratedRoute && (
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <Clock className="h-3 w-3" />
              <span>
                Last route created: {formatDateTime(lastAutoGeneratedRoute.createdAt)}
              </span>
            </div>
          )}
        </div>
              ...
              filtered.map((route) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* View Report / Monitor button for completed or in-progress routes */}
                    {(route.status === "completed" || route.status === "in_progress") && (
                      <button
                        ...
                        className={cn(
                          "mt-1.5 flex w-full items-center justify-center gap-1.5 rounded-md px-2 py-1 text-[10px] font-medium transition-colors",
